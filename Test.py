...
def metrics_new(G):
    ...
    pos_list_x = [G.nodes[i]['pos'] for i in range(len(G.nodes()))]
    pos_list_y = [0 for i in range(len(G.nodes()))]
    pos_list = list(zip(pos_list_x, pos_list_y))
    print(list(nx.isolates(G)))
    print(pos_list)
    print(n_iso, n_gaps)
    nx.draw(G, pos=pos_list, node_size=20, with_labels=True)
    plt.show()
...
def run_sims(ntrials, conn_fun, eta, L, Torus, mu, new=False, rt=1):
    ...
    if not os.path.isfile(fname):
        ...
        with open(fname, "w") as my_file:
            ...
            if mulabel in list(data.keys()):
                lastcount, last_pfc, last_iso, last_gap, last_isoUucg, last_split, last_degree, last_n_iso, last_n_gaps = \
                data[
                    mulabel]
    ...
    print(f"P_fc = {P_fc}, P_iso = {P_iso}, P_gap = {P_gap}, P_split = {P_split}, mean_degree = {mean_degree},"
          f" n_iso = {n_iso}, n_gaps = {n_gaps}")

mu_list_overall = [0.15, 0.2, 0.25, 0.3, 0.35]

# mu_list = mu_list_overall[0:1]
# mu_list = mu_list_overall[1:2]
# mu_list = mu_list_overall[2:3]
mu_list = mu_list_overall[3:5]

for mu in [0.6]:
    run_sims(1, "Waxman", 1.0, 100, False, mu, True)
