...
def PPP(rt, L):
    ...
    bulk = 500
    L_temp = float(L) + bulk
    n_nodes = random.poisson(rt * L_temp)  # Poisson number of points
    node_locations = sorted(random.uniform(-bulk/2, L + bulk/2, n_nodes))  # Uniformly distributed along the line
    boundary_nodes = [x for x in range(n_nodes) if not 0 <= node_locations[x] <= L]

    return node_locations, n_nodes, boundary_nodes
...
def edges(rt, L, mu, eta):
    ...
    pos_list, n_nodes, boundary_nodes = PPP(rt, L)
    node_list = [i for i in range(n_nodes)]
    edge_list = []

    for i in range(n_nodes):
        for j in range(i + 1, n_nodes, 1):
            if conn_fun(pos_list[i], pos_list[j], mu, eta) > 10 ** (-7):  # Checks that nodes aren't too far apart, if they are then break
                if random.uniform(0, 1) < conn_fun(pos_list[i], pos_list[j], mu, eta):
                    edge_list.append((i, j))
            else:
                break

    return edge_list, node_list, pos_list, boundary_nodes
...
def metrics(G, boundary_nodes):
    ...
    # mean_degree_temp = [list(dict(G.degree()).values())[i] for i in range(len(list(dict(G.degree()).values()))) if
    #                   list(dict(G.degree()))[i] not in boundary_nodes]
    # mean_degree_2 = sum(mean_degree_temp) / len(mean_degree_temp)  # Overall degree of the network divided by the number of nodes
    ...
    if nx.is_connected(G):
        ...
        bulk = [x for x in list(G.nodes()) if x not in boundary_nodes]
        iso_boundary = [i for i in list(nx.isolates(G)) if i in boundary_nodes]
        b_temp = list(nx.node_boundary(G, bulk, boundary_nodes))
        boundary_remove = [x for x in boundary_nodes if x not in b_temp]
        G.remove_nodes_from(iso_boundary + boundary_remove)
...
def run_sims(ntrials, conn_fun, eta, L, Torus, mu, rt=1):
    """
    ...
    while count < ntrials:
        ...
        if conn_fun == "Waxman":
            ...
            eta = eta
        edge_list, node_list, pos_list, boundary_nodes = edges(rt, L, mu, eta)
    ...
    if Torus == True:
        fname = "Simulated_Data/Remove_Boundary_Effects/%s/Eta_%s/L_%s/Torus/Overall.txt" % (conn_fun, eta, L)
        path = fname[:-12]
